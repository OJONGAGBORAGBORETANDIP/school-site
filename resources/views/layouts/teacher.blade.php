...
<head>
    ...
    <title>@yield('title', 'Teacher') – {{ config('app.name') }}</title>
    ...
    <div class="min-h-screen flex">
        ...
        <aside class="w-64 shrink-0 bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 flex flex-col">
            <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
                <h1 class="text-lg font-bold text-indigo-600 dark:text-indigo-400">Teacher Portal</h1>
                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">{{ config('app.name') }}</p>
            </div>
            <nav class="flex-1 mt-4 px-3 space-y-0.5">
                <a href="{{ route('teacher.dashboard') }}" class="flex items-center px-3 py-2 text-sm rounded-md {{ request()->routeIs('teacher.dashboard') ? 'bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 font-medium' : 'text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700' }}">
        ...
        <div class="flex-1 flex flex-col min-w-0">
            ...
            <header class="shrink-0 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
                <div class="px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between">
                    <div>
                        <h2 class="text-lg font-semibold text-gray-900 dark:text-gray-100">@yield('header-title', $headerTitle ?? 'Teacher')</h2>
